...
# TARGET_DEVICE_NAME_1 = "F_YELL"
# TARGET_DEVICE_NAME_2 = "F_CYAN"
TARGET_DEVICE_NAME_1 = "F_RED"
TARGET_DEVICE_NAME_2 = "F_CYAN"
...
plot_1 = win.addPlot(title="NTC Data Over Time (F_YELL)")
...
plot_2 = win.addPlot(title="NTC Data Over Time (F_CYAN)")
...
def detection_callback(device, advertisement_data):
    ...
    if device.name == TARGET_DEVICE_NAME_1:
        handle_device_data(advertisement_data, bat_sense_list_1, ntc_data_list_1, max30205_data_list_1, time_data_list_1, time_data_1, plot_data_bat_1, plot_data_ntc_1, plot_data_max_30205_1, curve_bat_1, curve_ntc_1, curve_max_30205_1, marker_curve_1, "FSEMI1")
    
    elif device.name == TARGET_DEVICE_NAME_2:
        handle_device_data(advertisement_data, bat_sense_list_2, ntc_data_list_2, max30205_data_list_2, time_data_list_2, time_data_2, plot_data_bat_2, plot_data_ntc_2, plot_data_max_30205_2, curve_bat_2, curve_ntc_2, curve_max_30205_2, marker_curve_2, "FSEMI2")
...
def handle_device_data(advertisement_data, bat_sense_list, ntc_data_list, max30205_data_list, time_data_list, time_data, plot_data_bat, plot_data_ntc, plot_data_max_30205, curve_bat, curve_ntc, curve_max_30205, marker_curve, device_name):
    global time_0, marker_curve_time, marker_curve_range, counter_number

    if advertisement_data.manufacturer_data:
        data = advertisement_data.manufacturer_data[62945]

        # Extract data (assuming these are correct indices)
        data_channel_1 = bat_data = int.from_bytes(data[6:8], byteorder='little')
        bat_data = int.from_bytes(data[8:10], byteorder='little')
        ntc_data = int.from_bytes(data[10:12], byteorder='little')
        max_30205_data = int.from_bytes(data[12:14], byteorder='little')

        counter_number += 1
        print("Device: ", device_name, "Channel 0: ", data_channel_1, "Channel 1: ", bat_data, "Channel 2: ", ntc_data, "Channel 3: ", max_30205_data, "Count: ", counter_number)
...
async def start_scan():
    ...
    await scanner.stop()
    print("Scan stopped.")
    print(f"Number of samples for FSEMI1: {len(ntc_data_list_1)}")
    print(f"Number of samples for FSEMI2: {len(ntc_data_list_2)}")
    print(time.time())
...
def save_to_excel():
    if ntc_data_list_1 or ntc_data_list_2:
        ...
        df1 = pd.DataFrame({
            ...
            "GPIO offset (FSEMI1)": bat_sense_list_1,
            "NTC Data (FSEMI1)": ntc_data_list_1,
            "MAX30205 (FSEMI1)": max30205_data_list_1
        })

        df2 = pd.DataFrame({
            "Time (seconds)": time_data_list_2,
            "GPIO offset (FSEMI2)": bat_sense_list_2,
            "NTC Data (FSEMI2)": ntc_data_list_2,
            "MAX30205 (FSEMI2)": max30205_data_list_2
        })

        # Write both dataframes to separate sheets in Excel
        with pd.ExcelWriter(os.path.join(output_data_folder, file_path)) as writer:
            df1.to_excel(writer, sheet_name="F_YELL Data", index=False)
            df2.to_excel(writer, sheet_name="F_CYAN Data", index=False)
